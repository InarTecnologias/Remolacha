...
function reqListener (jsonFile) {
 dataJSON = jsonFile;
  app = new App();
}


var socket = io('http://192.168.0.171:8080');
...
socket.on('message', (msg) => {
	...
	reqListener(msg.data);
 });
socket.on('recibido', (msg) => {
    console.log('cambio: ' + msg.data);
    app.data.update( msg.data );
 });
...
App.prototype.onResize = function(e) {
	...
	this.renderer.domElement.setAttribute( 'style', 'width:' + this.containerEl.offsetWidth*1.2 + 'px; height:' + this.containerEl.offsetHeight + 'px' );
	this.camera.left = this.containerEl.offsetWidth / - 1.8;
	this.camera.right = this.containerEl.offsetWidth / 1.6;
	this.camera.top = this.containerEl.offsetHeight / 1.8;
	this.camera.bottom = this.containerEl.offsetHeight / - 1.8;
