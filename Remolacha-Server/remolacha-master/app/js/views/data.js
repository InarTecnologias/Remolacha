...
Data.prototype.step = function( time ){
	...
	sum = this.bigRadius + this.ringRadius;
	containerEl = document.getElementById('main');
	min = Math.min( containerEl.offsetWidth, containerEl.offsetHeight ) / 2.5;
	if(sum > min){
		this.bigRadius = this.bigRadius*(min/sum);
		this.ringRadius = this.ringRadius*(min/sum);
	}
